'use strict';

define(['app'], function (app) {

    var horarioService = function ($http,$sessionStorage,$location,$rootScope, Auth ) {
        var horarioFactory = {};
        var currentDate = $rootScope.currentTime;
        var d= currentDate.getDate();
        var m = currentDate.getMonth();
        var y = currentDate.getFullYear();
        var hoy=currentDate.getDay();
        var colors = ['Brown','Light','Chocolate','Crimson','RoyalBlue','DarkOliveGreen','DarkSlateGray','MediumSlateBlue','SlateBlue'];
        var accessLevels = routingConfig.accessLevels
            , userRoles = routingConfig.userRoles
            , currentUser = $sessionStorage['user'];

        horarioFactory.accessLevels = accessLevels;
        horarioFactory.userRoles = userRoles;
        horarioFactory.user = currentUser;

        var nombreCompleto = function(apellido1, apellido2,nombre){

            if(apellido1===undefined){
                apellido1='';
            }
            if(apellido2===undefined){
                apellido2 = '';
            }
            if(nombre===undefined){
                nombre = '';
            }

            if(apellido1==undefined){
                apellido1='';
            }
            if(apellido2==undefined){
                apellido2 = '';
            }
            if(nombre==undefined){
                nombre = '';
            }

            if(apellido2=='.' || apellido2=='*'){
                apellido2="";
            }
            return apellido1+' '+apellido2+', '+nombre;
        };


        var enviarDataHorarioSeleccionadoHaciaAsistencia = function(selectHorarioEvent,lastWeek){
                
            if(lastWeek!=undefined && lastWeek!=null){
                $rootScope.lastWeek = lastWeek;
            }
            $rootScope.horaInicio = selectHorarioEvent.start.toDate();//hora inicio
            $rootScope.horaFin = selectHorarioEvent.end.toDate();//hora fin
            $rootScope.idCursoSeleccionado = selectHorarioEvent.idCurso;
            $rootScope.date = selectHorarioEvent.start.toDate();//fecha
            //$rootScope.horaInicio = selectHorarioEvent.start.subtract('days',7);
            //$rootScope.horaFin = selectHorarioEvent.end.subtract('days',7);
        };


        /*Obtiene los alumnos de un curso en particular del calendario de docente*/
         horarioFactory.obtenerAlumnosHorario = function (dia, horaInicio, horaFin, idCurso, $scope) {
            var cad="";            

            cad = $sessionStorage['JSESSION'];
            $sessionStorage['dia']=dia;
            $sessionStorage['horaInicio']=horaInicio;
            $sessionStorage['horaFin']=horaFin;
            $sessionStorage['idCurso']=idCurso;

            $http({
                method  : 'GET',
                url     : '/api/docente/horario/alumnos/'+dia+'/'+horaInicio+'/'+horaFin+'/'+idCurso,
                headers : { 'Cookie-REST': cad }  
            })
            .success(function(data, status) {
                if(status==200){
                    $scope.alumnos = data;
                    //console.log('obtenerAlumnosHorario');
                    //console.log(data);
                    $sessionStorage['alumnos']=$scope.alumnos;
                    //console.log($sessionStorage)
                }    
            });
        };
                
        function mostrarHorario($HorarioCurso,$modal,$scope,$compile) {

            //$(document).on('ready page on load',function(){

            var hdr = {};
            var view='agendaWeek';
            if($(window).width() < 514){
                    hdr = {left: 'title', center: '', right: 'prev,today,next'};
                    view = 'agendaDay';
            }
            else{
                if ($(window).width() <= 767) {
                    hdr = {left: 'title', center: 'agendaWeek,agendaDay', right: 'prev,today,next'};
                    view = 'agendaDay';
                } else {                    
                    hdr = {left: '', center: 'title', right: 'prev,today,agendaWeek,agendaDay,next'};
                    view = 'agendaWeek';                    
                }
            }
            
            /* initialize the calendar
            -----------------------------------------------------------------*/
            /*using spanish values*/           
        $('#calendar').fullCalendar({
                header: hdr,
                buttonText: {
                    prev: '<',
                    next: '>',
                    today: 'hoy',
                    month: 'mes',
                    week: 'semana',
                    day: 'dia'
                },
                titleFormat:{
                    month: 'MMMM YYYY',
                    week: 'MMMM',
                    day: 'DD MMMM YYYY'
                },
                columnFormat:{
                    month: 'ddd',
                    week: 'ddd',
                    day: 'dddd'
                },
                editable: false,
                allDaySlot: false,
                events: $HorarioCurso,
                contentHeight: 800,
                minTime: "07:00" ,
                maxTime: "23:00" ,
                defaultView: view,
                firstDay:1,
                hiddenDays:[0],
                eventColor: 'red',
                monthNames: ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Setiembre','Octubre','Noviembre','Diciembre'],
                monthNamesShort: ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Set','Oct','Nov','Dec'],
                dayNames: ['Domingo','Lunes','Martes','Miercoles','Jueves','Viernes','Sábado'],
                dayNamesShort: ['Dom','Lun','Mar','Mie','Jue','Vie','Sab'],
                now: currentDate,
                windowResize: function(event, ui) {
                    $('#calendar').fullCalendar('render');
                    if ($(window).width() <= 640) {
                        $('#calendar').fullCalendar( 'changeView', 'agendaDay' );
                    } else {
                        $('#calendar').fullCalendar( 'changeView', 'agendaWeek' );
                    }
                },
                eventClick: function(calEvent) {

                     //Si la fecha es anterior a la fecha actual del sistema
                    $sessionStorage['start'] = calEvent.start.toDate();
                    $sessionStorage['end'] = calEvent.end.toDate();
                    if(calEvent.start.isBefore(currentDate)){
                        calEvent.lastweek = false;

                        $sessionStorage['clase'] = calEvent;
                        $sessionStorage['clase'].source = null;    
                    }else{
                        calEvent.lastweek = true;                        
                    }   

                    enviarDataHorarioSeleccionadoHaciaAsistencia(calEvent,calEvent.lastweek);             

                },
                eventRender: function(event,element){
                    element.find('.fc-event-title').html(event.title);

                    //$(element).attr('ng-model','dropdown');
                    //$(element).attr('ng-change','enviarHaciaAsistencias()');
                    $(element).attr('template','/views/template/Asistencia/menuHorario.html');
                    $(element).attr('data-animation','');
                    $(element).attr('bs-dropdown','dropdown');
                    $(element).attr('data-html',true);
                    if( event.start.hour()*100>=2000){
                        $(element).attr('data-placement','top');
                    }
                    $compile(element)($scope);

                }
            });

            $('#calendar').fullCalendar('render');
            $("td.fc-header-left").append('<div class="hidden-xs"> <input type="image" src="/img/pdf_button.png" id="AE_btn_pdf" style="width:27px;height:27px;" /> Descargar Horario</div>');
            $("#AE_btn_pdf").click(function () {
              //#AEFC is my div for FullCalendar
                html2canvas($('#calendar'), {
                    logging: false,
                    useCORS: false,
                        onrendered: function (canvas) {
                            var imgData = canvas.toDataURL("image/png");
                            var cordonBleuLogoImg = "data:image/png;base64,";
                            var view_calendar = $('#calendar').fullCalendar( 'getView' );
                            var orientation = "landscape";
                            var verticalMargin = 20;
                            var nombreCompletoDocente = nombreCompleto(Auth.user.apellidoPaterno,Auth.user.apellidoMaterno,Auth.user.nombre);

                            if(view_calendar.title == 'agendaDay'){
                                orientation = "portrait";   
                            }
                            var doc = new jsPDF(orientation,"pt" ,"a4"); 

                            doc.setFontSize(18);
                            doc.text('Horario Docente', 380, 25+verticalMargin);
                            doc.setFontSize(15);
                            doc.text('Docente:' , 50, 60+verticalMargin);
                            doc.text('Generado:' , 50, 80+verticalMargin);

                            doc.text(Auth.user.codigo +" - "+nombreCompletoDocente , 150, 60+verticalMargin);
                            doc.text(moment($rootScope.currentTime).format('dddd, D MMMM YYYY, h:mm a'), 150, 80+verticalMargin);

                            doc.addImage(imgData, 'JPEG', 45, 120, 750, 450);
                            doc.addImage(cordonBleuLogoImg, 'PNG', 40, 15, 100, 30);
                            doc.output('save', 'horarioDocente.pdf');    
                                             
                        }
                }) ;      
           }); 
        }


       
       /*Obtiene campos a mostrar en el calendario de docente*/
       horarioFactory.obtenerHorariosDocente = function ($scope,$compile) {
            var cad = "";        

            cad = $sessionStorage['JSESSION'];

            $http({
                method : 'GET',
                url : '/api/docente/horario',
                headers : { 'Cookie-REST': cad }
            })
            .success(function(data, status) {
                if(status==200){

                    //console.log(data);
                    $scope.horario = data;
                    angular.copy($scope.horario, $scope.copy_horario);           


                    require( [ 'fullCalendar' ],function(){

                    $(function() {
                          
                          var currentReg=null;
                          var contador=1;
                          var horariosAgrupados = [];
                          var hoy_temp=hoy;

                        if(hoy_temp==0){
                            hoy_temp= hoy_temp + 7;
                        }


                        for (var i = 0 ; i < data.length; i++) {
                            var reg= {};
                            var insertar=false;

                            reg.id = contador;
                            reg.idHorario = data[i].idHorario;
                            reg.nombreCurso=data[i].curso.nombre;
                            reg.inicio=data[i].horaInicio;
                            reg.fin =data[i].horaFin;
                            reg.diaCurso=data[i].dia;
                            reg.dia=d+ reg.diaCurso - hoy_temp;                        
                            reg.codigoPeriodo = data[i].aiHorarioVacante.aiMallaCurricular.aiPeriodo.codigo;
                            reg.idCurso = data[i].curso.idCurso;
                            reg.tipoCurso = data[i].tipoSesion;
                            reg.idAula = data[i].aula.idAula;
                            reg.codAula = data[i].aula.codigo;
                            reg.idCarrera = data[i].aiHorarioVacante.aiCarreraSeccion.aiCarrera.idCarrera
                            reg.secciones = [];

                            reg.secciones.push(reg.idCarrera + "-"
                                + data[i].aiHorarioVacante.aiCarreraSeccion.idCarreraSeccion
                                + data[i].aiHorarioVacante.id.grupo);

                            if(currentReg==null){
                                currentReg = reg;
                            }

                            if(i+1<data.length){
                                var horarioInicioSiguiente = data[i+1].horaInicio;
                                var horarioFinSiguiente = data[i+1].horaFin; 
                                var idCursoSiguiente = data[i+1].curso.idCurso;
                                var idAulaSiguiente = data[i+1].aula.idAula;
                                var idCarreraSiguiente = data[i+1].aiHorarioVacante.aiCarreraSeccion.aiCarrera.idCarrera;
                                var diaCursoSiguiente = data[i+1].dia;
                                var tipoCurso = data[i+1].tipoSesion;

                                if( idCursoSiguiente ==   currentReg.idCurso
                                     && idAulaSiguiente == currentReg.idAula
                                     && diaCursoSiguiente == currentReg.diaCurso
                                     && tipoCurso == currentReg.tipoCurso){

                                    if(horarioInicioSiguiente == currentReg.inicio && 
                                        horarioFinSiguiente == currentReg.fin ){
                                        reg.secciones.push(idCarreraSiguiente + "-"
                                        + data[i+1].aiHorarioVacante.aiCarreraSeccion.idCarreraSeccion
                                        + data[i+1].aiHorarioVacante.id.grupo);
                                    }else{
                                        insertar=true;
                                    }              


                                }else{
                                    insertar=true;

                                }

                            }else{
                                insertar=true;
                            }

                            //console.log(i+" - "+insertar);
                            if(insertar){
                                horariosAgrupados.push(currentReg);
                                currentReg=null;
                                contador++;
                            }


                        }


                        var currentReg=null;
                        var mapCursoColorCount=0;
                        var HorarioCurso=new Array();
                        var mapCursoColor = [];
                        for (var i = 0 ; i < horariosAgrupados.length; i++) {
                            var insertar=false;
                            var reg= {};

                            reg = horariosAgrupados[i];

                            //console.log(reg.diaCurso);

                            if(currentReg==null){
                                currentReg = reg;
                            }

                            if(i+1<horariosAgrupados.length){
                                var horarioSiguiente = horariosAgrupados[i+1];
                                var seccionesIguales = true;
                                
                                if(horarioSiguiente.diaCurso == reg.diaCurso 
                                   && horarioSiguiente.idCurso == reg.idCurso
                                   && horarioSiguiente.tipoCurso == reg.tipoCurso
                                   && horarioSiguiente.idAula == reg.idAula
                                   &&  reg.fin == horarioSiguiente.inicio ){

                                    angular.forEach (currentReg.secciones, function(r){
                                        var elementoEncontrado = false;
                                        for(var j=0;j<horarioSiguiente.secciones.length;j++){
                                            if(r == horarioSiguiente.secciones[j]){
                                                elementoEncontrado = true;
                                                break;                                            
                                            }
                                        }

                                        if(!elementoEncontrado){
                                            seccionesIguales = false;                                        
                                        }
                                    });

                                }else{
                                    seccionesIguales=false;
                                }                                                             


                                if(!seccionesIguales){
                                    insertar=true;
                                } else{
                                    currentReg.fin = horarioSiguiente.fin;
                                }                              

                            }else{
                                insertar=true;
                            }
                            
                            
                            if(insertar){                              

                                if(mapCursoColor[currentReg.idCurso] ==null || mapCursoColor[currentReg.idCurso] ==undefined){
                                    mapCursoColor[currentReg.idCurso] = colors[mapCursoColorCount]; 
                                    mapCursoColorCount++;                                 
                                }

                                var temp = {
                                    'id':  currentReg.idHorario,
                                    'title': currentReg.nombreCurso+'  <strong>[ '+currentReg.tipoCurso+' ]'+'</strong><br/>'+currentReg.codAula,
                                    'start': new Date(y, m, currentReg.dia , currentReg.inicio/100, currentReg.inicio%100),
                                    'end': new Date(y,m,currentReg.dia ,currentReg.fin/100,currentReg.fin%100),
                                    'allDay': false,
                                    'periodo': currentReg.codigoPeriodo,
                                    'idCurso': currentReg.idCurso,
                                    'tipoCurso': currentReg.tipoCurso,
                                    'color' : mapCursoColor[currentReg.idCurso]
                                };
                                HorarioCurso.push(temp);
                                currentReg=null;

                            }

                        }

                        //console.log(HorarioCurso);

                    
                        mostrarHorario(HorarioCurso,$scope.getModal(),$scope,$compile);
                     
                        });
                    });                
                }
            });

        };

        return horarioFactory;

    };

    app.factory('HorarioService', ['$http','$sessionStorage','$location','$rootScope','Auth',horarioService]);

});